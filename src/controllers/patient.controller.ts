...
import {
  calculateLunarPhase,
  calculateZodiacSamutthana,
} from "../services/lunar/lunar.service";
 ...
 */
export const searchByNationalId = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    if (!nationalId) {
      res
        .status(400)
        .json({ success: false, message: "กรุณาระบุเลขบัตรประชาชน" });
      return;
    }
    const { rows } = await db.query(
      `SELECT p.*,
    ...
    if (!rows.length) {
      res
        .status(404)
        .json({ success: false, message: "ไม่พบข้อมูลคนไข้", data: null });
      return;
    }
    const row = rows[0];
    const vs =
...
export const create = async (
  ...
  try {
    ...
    if (!national_id || national_id.length !== 13) {
      res
        .status(400)
        .json({ success: false, message: "เลขบัตรประชาชนต้องมี 13 หลัก" });
      return;
    }
    ...
    if (existing.rows.length) {
      res
        .status(409)
        .json({ success: false, message: "มีข้อมูลผู้ป่วยรายนี้ในระบบแล้ว" });
      return;
    }
    ...
    let effectiveLunarMonth = lunar_month
      ? parseInt(String(lunar_month), 10)
      : null;
    if (!effectiveLunarMonth && lunar_birthday) {
      const match = String(lunar_birthday).match(/เดือน\s*([0-9๑-๙]+)/);
      if (match) {
        const thaiDigits = "๐๑๒๓๔๕๖๗๘๙";
        const numStr = match[1].replace(/[๐-๙]/g, (d: string) =>
          String(thaiDigits.indexOf(d)),
        );
    ...
    const effBirthElement =
      birth_zodiac_element ?? zodiacCalc?.birthZodiacElement ?? null;
    const effBirthSamutthana =
      birth_samutthana ?? zodiacCalc?.birthSamutthana ?? null;
    const effBirthRakon = birth_rakon ?? zodiacCalc?.birthRakon ?? null;
    const effBirthDesc =
      birth_element_desc ?? zodiacCalc?.birthElementDesc ?? null;

    const effConceptionMonth =
      conception_lunar_month ?? zodiacCalc?.conceptionLunarMonth ?? null;
    const effConceptionZodiac =
      conception_zodiac ?? zodiacCalc?.conceptionZodiac ?? null;
    const effConceptionElement =
      conception_zodiac_element ?? zodiacCalc?.conceptionZodiacElement ?? null;
    const effConceptionSamutthana =
      conception_samutthana ?? zodiacCalc?.conceptionSamutthana ?? null;
    const effConceptionRakon =
      conception_rakon ?? zodiacCalc?.conceptionRakon ?? null;
    const effConceptionDesc =
      conception_element_desc ?? zodiacCalc?.conceptionElementDesc ?? null;
    ...
    const { rows } = await db.query(
      ...
      [
        ...
        effConceptionMonth,
        effConceptionZodiac,
        effConceptionElement,
        effConceptionSamutthana,
        effConceptionRakon,
        effConceptionDesc,
      ],
    );
...
export const update = async (
  ...
  try {
    ...
    const effectiveAllergy =
      drug_allergy !== undefined
        ? drug_allergy
        : allergy_notes !== undefined
          ? allergy_notes
          : current.drug_allergy;
    const effectiveFoodAllergy =
      food_allergy !== undefined ? food_allergy : current.food_allergy;
    const formattedBirthTime =
      birth_time !== undefined
        ? birth_time
          ? formatTimeToHHMMSS(birth_time)
          : null
        : current.birth_time;
    ...
    const effectiveBirthDayOfWeek =
      birth_day_of_week !== undefined
        ? birth_day_of_week
        : current.birth_day_of_week;
    ...
    const effectiveChronicDisease =
      chronic_disease !== undefined ? chronic_disease : current.chronic_disease;
    const effectiveGeography =
      geography !== undefined ? geography : current.geography;
    const effectiveLunarBirthday =
      lunar_birthday !== undefined ? lunar_birthday : current.lunar_birthday;
    const effectiveBodyElement =
      body_element !== undefined ? body_element : current.body_element;
    const effectiveLunarMonth =
      lunar_month !== undefined ? lunar_month : current.lunar_month;
    const effectiveZodiacYear =
      zodiac_year !== undefined ? zodiac_year : current.zodiac_year;
    const effectiveBirthZodiac =
      birth_zodiac !== undefined ? birth_zodiac : current.birth_zodiac;
    const effectiveBirthZodiacElement =
      birth_zodiac_element !== undefined
        ? birth_zodiac_element
        : current.birth_zodiac_element;
    const effectiveBirthSamutthana =
      birth_samutthana !== undefined
        ? birth_samutthana
        : current.birth_samutthana;
    const effectiveBirthRakon =
      birth_rakon !== undefined ? birth_rakon : current.birth_rakon;
    const effectiveBirthElementDesc =
      birth_element_desc !== undefined
        ? birth_element_desc
        : current.birth_element_desc;
    const effectiveConceptionLunarMonth =
      conception_lunar_month !== undefined
        ? conception_lunar_month
        : current.conception_lunar_month;
    const effectiveConceptionZodiac =
      conception_zodiac !== undefined
        ? conception_zodiac
        : current.conception_zodiac;
    const effectiveConceptionZodiacElement =
      conception_zodiac_element !== undefined
        ? conception_zodiac_element
        : current.conception_zodiac_element;
    const effectiveConceptionSamutthana =
      conception_samutthana !== undefined
        ? conception_samutthana
        : current.conception_samutthana;
    const effectiveConceptionRakon =
      conception_rakon !== undefined
        ? conception_rakon
        : current.conception_rakon;
    const effectiveConceptionElementDesc =
      conception_element_desc !== undefined
        ? conception_element_desc
        : current.conception_element_desc;
    ...
    await db.query(
      ...
      [
        ...
        effectiveBirthDayOfWeek,
        formattedBirthTime,
        effectiveHouseNo,
        effectiveMoo,
        effectiveRoad,
        effectiveSubdistrict,
        effectiveDistrict,
        effectiveProvince,
        effectiveZipcode,
        effectivePmh,
        effectiveChronicDisease,
        effectiveGeography ?? null,
        effectiveLunarBirthday,
        effectiveBodyElement,
        effectiveLunarMonth ?? null,
        effectiveZodiacYear ?? null,
        effectiveBirthZodiac ?? null,
        effectiveBirthZodiacElement ?? null,
        effectiveBirthSamutthana ?? null,
        effectiveBirthRakon ?? null,
        effectiveBirthElementDesc ?? null,
        effectiveConceptionLunarMonth ?? null,
        effectiveConceptionZodiac ?? null,
        effectiveConceptionZodiacElement ?? null,
        effectiveConceptionSamutthana ?? null,
        effectiveConceptionRakon ?? null,
        effectiveConceptionElementDesc ?? null,
